...
def  main():
  ...
  installer = "\installer\BassGen Installer.exe"
   
  if demo:
    installer = "\installer\BassGen Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BassGen manual.pdf" 
  ]
  ...
  ZIPNAME = "BassGen-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BassGen-v" + FULLVERSIONSTR + "-win-demo.zip"
