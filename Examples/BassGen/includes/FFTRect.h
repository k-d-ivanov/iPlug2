...
public:
		...
		: IControl(pPlug, pR), mColor(c), mParam(par), line(l)
	{
		...
		maxFreq = 44100. / 0.5;
		sampleRate = 44100.;
		width = static_cast<int>(mDrawRECT.W());
		value.resize(sz / 2 + 1);
		iVal.resize(width);
		iPeak.resize(width);
		OctaveGain = 1.;
	...
	{
		double x, y, yPeak;
		double xPrev = mDrawRECT.L;
		double yPrev = mDrawRECT.B;
		...
		for (int b = 0; b < width; b++){
			...
			y = RangeConvert(BOUNDED(dbv, dBFloor, 0.), 0., (double)mDrawRECT.T, dBFloor, (double)mDrawRECT.B);
			double pdv = AmpToDB(iPeak[b] * gainO);
			yPeak = RangeConvert(BOUNDED(pdv, dBFloor, 0.), 0., (double)mDrawRECT.T, dBFloor, (double)mDrawRECT.B);

			if (!line) pGraphics->DrawVerticalLine(&mColor2, x, mDrawRECT.B, y);
...
public:
	...
	bool Draw(IGraphics* pGraphics){
		for (int i = 0; i < HLineDraw.size(); i++) {
			pGraphics->DrawVerticalLine(&mColor, HLineDraw[i], mDrawRECT.B - 20, mDrawRECT.T);
		}

		for (int i = 0; i < FreqTxtDraw.size(); i++) {
			char hz[7];
			std::strcpy(hz, FreqTxtDraw[i].freq.c_str());
			IRECT box(FreqTxtDraw[i].pos - (space / 2), mDrawRECT.B - 15, FreqTxtDraw[i].pos + (space / 2), mDrawRECT.B);
...
private:
	void CalcSpacing() {
		...
		for (int i = minFreq10; i < maxFreq; i += 10) {
			const int bin = (int)((std::log10((double)i / (double)minFreq) / (std::log10((double)maxFreq / (double)minFreq))) * (double)(mDrawRECT.W()));
			if (i < 80 && i % 10 == 0) {
				if (bin > prevTxt + space && bin < mDrawRECT.W() - space / 2) {
					addToList = true;
					s << i;
				}
			}
